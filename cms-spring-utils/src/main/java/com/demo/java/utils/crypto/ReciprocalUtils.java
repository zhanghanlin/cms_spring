...
import com.demo.java.utils.string.StringGenerator;

public final class ReciprocalUtils {
    final static Logger logger = LoggerFactory.getLogger(ReciprocalUtils.class);
    ...
    private static boolean keyLengthCheck(String secretKey, String algorithm) {
        ...
        if (algorithm.equals(Algorithm.DES)) {
            if (secretKey.length() == 8) {
                ...
                logger.debug("key length is " + secretKey.getBytes().length + " ,must lequal 8");
            }
        } else if (algorithm.equals(Algorithm.DESede)) {
            if (secretKey.length() == 24) {
                lengthCheck = true;
            } else {
                logger.debug("key length is " + secretKey.getBytes().length + " ,must equal 24");
            }
        } else if (algorithm.equals(Algorithm.AES)) {
            if (secretKey.length() == 16) {
                lengthCheck = true;
            } else {
                logger.debug("key length is " + secretKey.getBytes().length + " ,must equal 16");
            }
        } else if (algorithm.equals(Algorithm.Blowfish)) {
            if (secretKey.length() <= 16) {
                lengthCheck = true;
            } else {
                logger.debug("key length is " + secretKey.getBytes().length + " ,must be less then 16");
            }
        }
        return lengthCheck;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(ReciprocalUtils.generateKey(ReciprocalUtils.Algorithm.DES));
    }
}
